...
if (! class_exists ( 'DTCoursesPostType' )) {
	class DTCoursesPostType {
			...
			if(function_exists( 'is_woocommerce' )) {
				...
				if($payment_method == 'woocommerce') {
					...
					if(dttheme_yith_subscription_plugin_active()) {
						...
						add_action ( 'add_meta_boxes', array (
								$this,
								'dt_course_subscription_product_metabox' 
						) );
		...
		function dt_course_product_metabox() {
			add_meta_box ( "dt-course-product-metabox", __ ( 'WooCommerce Product', 'dt_themes' ), array (
					$this,
					'dttheme_course_product_metabox_function' 
			), 'dt_courses', "side", "core" );
		}
		
		function dt_course_subscription_product_metabox() {
			add_meta_box ( "dt-course-subscription-product-metabox", __ ( 'WooCommerce Subscription Product', 'dt_themes' ), array (
					$this,
					'dttheme_course_subscription_product_metabox_function' 
			), 'dt_courses', "side", "core" );
		}
		
		function dt_events_calendar_metabox() {
			add_meta_box ( "dt-events-calendar-metabox", __ ( 'Course Events', 'dt_themes' ), array (
					$this,
					'dttheme_events_calendar_metabox_function' 
			), 'dt_courses', "side", "core" );
		}
		...
		function dttheme_course_product_metabox_function() {
			include_once plugin_dir_path ( __FILE__ ) . 'metaboxes/dt_course_product_metabox.php';
		}
		
		function dttheme_course_subscription_product_metabox_function() {
			include_once plugin_dir_path ( __FILE__ ) . 'metaboxes/dt_course_subscription_product_metabox.php';
		}
		
		function dttheme_events_calendar_metabox_function() {
			include_once plugin_dir_path ( __FILE__ ) . 'metaboxes/dt_events_calendar_metabox.php';
		}
		...
		function dt_courses_columns_display($columns, $id) {
			...
			switch ($columns) {
				...
				case "woocommerce-product":
					$product_id = get_post_meta( $id, 'dt-course-product-id', true );
					if($product_id != '') {
				...
				case "woocommerce-subscription-product":
					$product_ids = get_post_meta( $id, 'dt-course-subscription-product-id', true );
		...
		function save_post_meta($post_id) {
			...
			if(isset($_POST['layout'])) :
				...
				if( isset( $_POST ['dt-course-product-id'] ) && $_POST ['dt-course-product-id'] != '' ) {
					update_post_meta ( $post_id, 'dt-course-product-id',  $_POST ['dt-course-product-id'] );
				} else {
					delete_post_meta ( $post_id, 'dt-course-product-id' );
				}

				if( isset( $_POST ['dt-course-subscription-product-id'] ) && $_POST ['dt-course-subscription-product-id'] != '' ) {
					update_post_meta ( $post_id, 'dt-course-subscription-product-id',  $_POST ['dt-course-subscription-product-id'] );
				} else {
					delete_post_meta ( $post_id, 'dt-course-subscription-product-id' );
				}
				
				if( isset( $_POST ['dt-course-event-id'] ) && $_POST ['dt-course-event-id'] != '' ) {
					update_post_meta ( $post_id, 'dt-course-event-id',  $_POST ['dt-course-event-id'] );
				} else {
					delete_post_meta ( $post_id, 'dt-course-event-id' );
				}
