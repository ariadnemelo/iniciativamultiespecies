...
$_POST['dt_complete_lesson_noonce'] = isset($_POST['dt_complete_lesson_noonce']) ? $_POST['dt_complete_lesson_noonce']: '';
...
?>
	...
	if ( dttheme_check_if_user_authorized_to_view_course($dt_lesson_course) || $private_lesson == '' ){
		   ...
		   <div class="column dt-sc-two-third first dt-sc-lessons-content-container">
				  ...
				  <?php
					...
					<?php 
						...
						if($duration > 0) {
							...
							if($hours == 0) {
								$duration = sprintf( _n( '%d min', '%d mins', $mins, 'dt_themes' ), $mins );				
							} else {
								$duration = sprintf( _n( '%d hour', '%d hours', $hours, 'dt_themes' ), $hours ).' '.sprintf( _n( '%d min', '%d mins', $mins, 'dt_themes' ), $mins );
							}							
						}
					?>
				...
				if(isset($lesson_video) && $lesson_video != '') { ?>
					...
					<div class="lesson-video">
						...
						if(wp_oembed_get( $lesson_video ) != '') {
							echo wp_oembed_get( $lesson_video ); 
						} else {
							echo wp_video_shortcode( array('src' => $lesson_video) );
						}
						?>
                    </div>
				<?php } ?>
			   ...
			   <div class="dt-sc-hr-invisible-small"></div>
				...
				if(isset($dt_lesson_quiz) && $dt_lesson_quiz > 0) {
					...
					if(is_user_logged_in()) {
						...
						if(!isset($dt_grade_post[0]) && (!isset( $_POST['complete_lesson']) && !wp_verify_nonce($_POST['dt_complete_lesson_noonce'], 'dt_complete_lesson_noonce'))) {
			...
			<div class="column dt-sc-one-third dt-sc-lessons-menu-container">
				...
				<div class="dt-sc-lessons-menu">
					...
					if(isset($lessons_hierarchy_array[0]) && count($lessons_hierarchy_array[0]) > 0) {
						echo '<ul>';
						foreach ($lessons_hierarchy_array[0] as $lesson_item){
							...
							echo '<li class="'.$dt_lesson_status.$current_cls.'"><a href="'.get_permalink($lesson_item->ID).'" class="dt-sc-lesson-menu-name '.$dt_tooltip_dir.'" '.$dt_tooltip_title.'>'.$lesson_item->post_title.'</a>';
								...
								echo dttheme_get_course_curriculum_lesson_details( $lessons_hierarchy_array,  $lesson_item->ID, $s2_level, $the_id );
				...
				<div class="dt-sc-lesson-staff">
				  <div class="lesson-staff-title">
					<h5><?php echo $GLOBALS['teachers-singular-label']; ?></h5>
				  </div>
				  <div class="lesson-staff-details">
					<div class="team-thumb"> 
						<?php if( has_post_thumbnail($teacher_data->ID) ):
								...
								echo '<img src="http'.dttheme_ssl().'://placehold.it/500x500&text=Image" alt="" />';
						endif; ?>
					</div>
					<div class="team-meta"> 
		...
		echo '<div class="dt-sc-warning-box">';
			echo sprintf( __('You have to purchase the course %s to view this lesson.', 'dt_themes'), '<a href="'.get_permalink($dt_lesson_course).'">'.get_the_title($dt_lesson_course).'</a>' );
		echo '</div>';
