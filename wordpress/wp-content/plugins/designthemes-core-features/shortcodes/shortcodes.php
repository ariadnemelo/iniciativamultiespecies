...
class DTCoreShortcodesDefination {
	...
	function __construct() {
		...
		/* Whatsapp No */
		add_shortcode ( "dt_sc_whatsapp_no", array ( $this, "dt_sc_whatsapp_no") );
		...
		/* Class Shortcode */
		add_shortcode ( "dt_sc_classes", array ( $this, "dt_sc_classes" ) );
		
		/* Quizzes Shortcode */
		add_shortcode ( "dt_sc_quizzes", array ( $this, "dt_sc_quizzes" ) );
	 ...
	 * @return multitype:
	 */
	static function dtListImages($dir = null) {
		$images = array ();
	...
	function dt_sc_phone($attrs, $content = null) {
		...
		$out .= ( !empty($phone) ) ?"<p><a href='tel:{$href}'>{$phone}</a></p>": "";
		$out .= "<span></span>";
		$out .= '</div>';
	...
	function dt_sc_mobile($attrs, $content = null) {
		...
		$href = preg_replace('/[^0-9]/', '', $mobile);
		
		$out = '<div class="dt-sc-contact-info">';
		$out .= "<div class='icon'><i class='fa fa-mobile-phone'></i></div>";
		$out .= ( !empty($mobile) ) ?"<p><a href='tel:{$href}'>{$mobile}</a></p>": "";
		$out .= "<span></span>";
		$out .= '</div>';
	...
	function dt_sc_whatsapp_no($attrs, $content = null) {
		...
		$out = '<div class="dt-sc-contact-info">';
			$out .= "<div class='icon'><i class='{$class}'></i></div>";
			$out .= ( !empty($text) ) ?"<p><a target='_blank' href='https://api.whatsapp.com/send?phone={$href}'> {$text} </a></p>": "";
			$out .= "<span></span>";
		$out .= '</div>';
	...
	function dt_sc_icon_box($attrs, $content = null, $shortcodename = "") {
		...
		if( !empty($fontawesome_icon) ){
			$out .= "<div class='custom-icon' {$style}><a href='{$link}' target='_blank'><span class='fa fa-{$fontawesome_icon}'></span></a></div>";
		} elseif( !empty($stroke_icon) ){
			$out .= "<div class='custom-icon' {$style}><a href='{$link}' target='_blank'><span class='icon $stroke_icon'></span></a></div>";
		}elseif( !empty($custom_icon) ){
			$out .= '<div class="custom-icon" '.$style.'><span><a href="'.$link.'" target="_blank"><img src="'.$custom_icon.'" title="'.$title.'" alt="'.$title.'"></a></span></div>';
		}
	...
	function dt_sc_team($attrs, $content = null) {
		...
		$attributes = array (
				'name' => '',
				'image' => 'http'.dttheme_ssl().'://placehold.it/300',
				'role' => '',
				'alt' => '',
				'title' => '',
				'type' => ''
	...
	function dt_sc_testimonial($attrs, $content = null) {
		extract ( shortcode_atts ( array ( 'name' => '', 'role' => '', 'type' => '', 'enable_rating' => '', 'class' => '','image' => 'http'.dttheme_ssl().'://placehold.it/300'), $attrs ) );
		$content = DTCoreShortcodesDefination::dtShortcodeHelper ( $content );
		if($type == 'type3') $content = ! empty ( $content ) ? '<q><span>"</span>'.$content.'<span>"</span></q>' : "";
		else $content = ! empty ( $content ) ? '<q> "'.$content.'"</q>' : "";
	...
	function dt_sc_title( $attrs,$content = null , $shortcodename = "" ){
		extract ( shortcode_atts ( array ( 'class' => '', 'type' => '' ), $attrs ) );

		$shortcodename = str_replace ( "dt_sc_", "", $shortcodename );
		$out = "<{$shortcodename} class='border-title {$class} {$type}'>";
		$out .= DTCoreShortcodesDefination::dtShortcodeHelper ( $content );
		$out .= "<span></span>";
		$out .= "</{$shortcodename}>";
		return $out;	
	...
	function dt_sc_portfolio_item( $attrs, $content = null, $shortcodename= "" ){
		...
		if( !empty( $id ) ){
			...
			if( $p->post_type === "dt_portfolios" ):
				...
				$out .= '<figure>';
							$popup = 'http'.dttheme_ssl().'://placehold.it/1170x878&text=Add%20Image%20/%20Video%20%20to%20Portfolio';
							if( array_key_exists('items_name', $portfolio_item_meta) ) {
								...
								if( "video" === $item ) {
									...
									if( !empty($items) ) {
										...
										$out .= '<img src="http'.dttheme_ssl().'://placehold.it/1170x878&text=Add%20Image%20/%20Video%20%20to%20Portfolio" alt="'.__('portfolio', 'dt_themes').'" width="1170" height="878"/>';
									}	
								} else {
						...
						$out .= '<figure>';
									...
									if( array_key_exists('items_name', $portfolio_item_meta) ) {
										...
										if( "video" === $item ) {
											...
											$out .= '<img src="'.$portfolio_item_meta['items'][0].'" alt="'.__('portfolio', 'dt_themes').'" />';
										}	
									} else{
	...
	function dt_sc_fullwidth_video( $attrs, $content = null ) {
		...
		$backgroundimage = !empty( $backgroundimage )  ? "$backgroundimage" : 'http'.dttheme_ssl().'://placehold.it/1920x400&text=DesignThemes';
		...
		$out .= '		<div class="entry-thumb">';
						...
						$out .= '<a href="'.esc_url($link).'">';
							if( has_post_thumbnail( $id )) {
								...
								$out .= '<img src="http'.dttheme_ssl().'://placehold.it/1170x822&text=Image" alt="'.$title.'" />';
							}
						$out .= "</a>";
						...
						$out .='<div class="entry-thumb-desc"><p>'.dttheme_excerpt($excerpt_length).'</p></div>';	
		...
		if($wp_query->have_posts()): $i = 1;
		 while($wp_query->have_posts()): $wp_query->the_post();
			...
			$out .= '			<h5><a href="'.$link.'" data-gal="prettyPhoto[pp_gal]">'.get_the_title().'</a></h5>';
								if(isset($teacher_settings['role']) && $teacher_settings['role'] != '')
						...
						$out .= '<a href="'.get_permalink().'" >';
							$out .= '<div class="dt-sc-course-thumb">';
								if(has_post_thumbnail()):
									...
									$out .= '<img src="http'.dttheme_ssl().'://placehold.it/1170x822&text=Image" alt="'.get_the_title().'" />';
								endif;
							$out .= '</div>';
						$out .= '</a>';
						
						$wooproductID = get_post_meta(get_the_ID(), '_course_woocommerce_product', true);
						if($wooproductID != '' && $wooproductID != '-') {
							$wc_product = wc_get_product( $wooproductID );
							$out .= '<a href="'.get_permalink().'" class="dt-sc-course-price">'.$wc_product->get_price_html().'</a>';
						} else {
	...
	function dt_sc_events( $atts, $content = null ) {
		...
		if(!function_exists('dt_events_list') && ( class_exists('Tribe__Events__Main') || class_exists('Tribe__Events__Pro__Main') ) ) {
				...
				$out .= '<'.$html_tag.' class="dt-sc-one-half column '.$firstcls.'" id="post-'.get_the_ID().'">';
					...
					$out .= '<div class="dt-sc-event-container">';
						$out .= '<div class="dt-sc-event-thumb">';
							$out .= '<a href="'.get_permalink().'" title="'.get_the_title().'">';
									if ( has_post_thumbnail()):
										...
										$out .= '<img src="http'.dttheme_ssl().'://placehold.it/1170x895&text=Image" alt="'.get_the_title().'" />';
									endif;		
							$out .= '</a>';
						...
						$out .= '<div class="dt-sc-event-content">';
							...
							$out .= '<p> <i class="fa fa-calendar-o"> </i>'.tribe_get_start_date($post->ID, false, 'M Y @ h i a').' - '.tribe_get_end_date($post->ID, false, 'M Y @ h i a').' </p>';
	...
	function dt_sc_courses( $atts, $content = null ) {
			...
			if(dttheme_option('general', 'disable-theme-default-courses') != 'true') {
				...
				if($carousel != 'true') {
					$out .= '<div class="dt-sc-course-list-container">';
				}
					...
					if($the_query->have_posts()): 
					 while($the_query->have_posts()): $the_query->the_post();
							...
							if($carousel == 'true' && ($carousel_column == '2' || $carousel_column == '3' || $carousel_column == '4')) {
							...
							$out .= '<article id="post-'.get_the_ID().'" class="'.implode(" ", get_post_class("dt-sc-custom-course-type {$layout_class}", get_the_ID())).'">';
								...
								$out .= '<div class="dt-sc-course-details">';
										...
										if( $layout_view == "list" ) {
										   ...
										   $out .= '<h5><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h5>';
											...
											if(dttheme_check_if_course_exists_in_class(get_the_ID())) {
												$out .= '<div class="dt-sc-course-class"><i class="fa fa-institution"></i>'.dttheme_get_course_classes_links(get_the_ID()).'</div>';
											}
										
											if($payment_method == 'woocommerce') {
												$out .= dttheme_get_course_details_linked_with_products( get_the_ID(), 'archive' );
											} else {
												$out .= dttheme_get_course_details_linked_with_s2member( get_the_ID(), 'archive' );
											}
										...
										if($enable_certificate == 'true' || $enable_badge == 'true') {
											$out .= '<div class="dt-sc-certificate-badge">';
												if($enable_certificate == 'true') {
													$out .= '<span class="certificate"></span>';	
												}
				...
				return '<div class="dt-sc-error-box">'.__('You have disabled theme default courses in Buddha Panel settings. Please enable it.', 'dt_themes').'</div>';
	...
	function dt_sc_courses_search( $atts, $content = null ) {
		...
		if(dttheme_option('general', 'disable-theme-default-courses') != 'true') {
			...
			$out .= '</div>';
			$out .= '</div>';
			$out .= '<div id="dt-sc-ajax-load-image" class="search-ajax-load" style="display:none;"><img src="'.plugin_dir_url ( __FILE__ ) . "images/loading.gif".'" alt="ajax-loader" /></div>';
			$out .= '</div>';
					...
					<div class="dt-sc-subscription-frm-container">
						...
						<form name="frmsubscription" action="'.get_template_directory_uri().'/framework/subscribe_mail.php" class="dt-sc-subscription-frm" method="post">
							...
							if($enable_planavisit != 'true') {
								...
								$check = apply_filters('dt_sc_visit_form_elements', $privacy, $attrs ); 
						...
						$out .= '<div id="ajax_subscribe_msg"></div>
								<button type="submit" class="dt-sc-button" value="'.$submit_text.'" name="btnsubscribe" id="btnsubscribe">'.$submit_text.'</button><input type="hidden" value="'.$admin_email.'" name="hid_adminemail">
						...
						$check = apply_filters('dt_sc_visit_form_elements', $privacy, $atts ); 
	...
	function dt_sc_subscribed_courses( $atts, $content = null ) {
		...
		if( defined('WS_PLUGIN__S2MEMBER_VERSION') ) {
			...
			if($hide_visit_count != 'true') {
				...
				$out .= '<h2 class="border-title">'.sprintf( esc_html__('Hi %1$s, this is your %2$s visit', 'dt_themes'), $display_name, $login_count.$count_suffix ).'<span> </span></h2>';
			...
			if(isset($user_ccaps) && !empty($user_ccaps)) {
				...
				foreach($user_ccaps as $ccap) {
					...
					$ccap = (int)str_replace('classid_', '', $ccap);
	...
	function dt_sc_newsletter_section( $atts, $content = null ) {
		...
		$out = '<section id="newsletter">
					...
					<form name="frmNewsletter" method="post" class="dt-sc-subscribe-frm">
						<input type="email" name="dt_mc_emailid" id="dt_mc_emailid" required="" placeholder="'.__('Your Email Address', 'dt_themes').'" />
						<input type="hidden" name="dt_mc_apikey" id="dt_mc_apikey" value="'.stripslashes(dttheme_option('general','mailchimp-key')).'" />
						<input type="hidden" name="dt_mc_listid" id="dt_mc_listid" value="'.stripslashes(dttheme_option('general','mailchimp-listid')).'" />
						<input type="submit" name="submit" class="dt-sc-button small" value="'.__('Subscribe', 'dt_themes').'" />';
		$check = apply_filters('dt_sc_mailchimp_form_elements', $privacy, $atts ); 
	...
	function dt_sc_slider_search( $atts, $content = null ) {
		...
		if(dttheme_get_page_permalink_by_its_template('tpl-courses-search.php') != '') {	
			...
			$action = esc_url(home_url( '/', dttheme_ssl()));
			$srch_str = 's';
		}
		...
		$course_args = array('post_type' =>  'dt_courses', 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC', 'suppress_filters' => 0);
		$course_array = get_posts( $course_args );
		$course_data = wp_list_pluck( $course_array, 'post_title' );
		$course_data = json_encode($course_data);
		
		$out = '<div class="slider-search">';
					if($disable_search != 'true') {
						$out .= '<form method="get" id="courses-search" class="courses-search" action="'.$action.'">                  	
									...
									<div class="course-title-holder hidden">'.$course_data.'</div>
								</form>';
					}
				if($title != '') 
					$out .= '<h4>'.$title.'</h4>';
	...
	function dt_sc_certificate($attrs, $content = null) {		
		...
		if($item == 'student_name') {
			...
			$dtgradings = array( 'post_type' => 'dt_gradings', 'meta_query'=>array(), 'orderby' => 'post_date', 'order' => 'DESC', 'suppress_filters' => 1 );
		...
		if($type == 'type1') {
			...
			$out .= '<div class="dt-sc-course-certificate-wrapper">
						<div class="dt-sc-course-certificate" '.$cert_bg.'>';                     
							...
							if($logo_topleft != '' || $logo_topright != '' || $certificate_title != '') {
									$out .= '<div class="dt-sc-cert-header">';
							...
							$out .= '<div class="dt-sc-cert-content">'.$content.'</div>';
							
							if($show_certificate_issueddate == 'yes' || $logo_bottomcenter != '' || $authority_sign != '') {
								
								 $out .= '<div class="dt-sc-cert-footer">';
             ...
             $out .= '<div class="dt-sc-course-certificate-wrapper type2">';
						...
						$out .= '<div class="dt-sc-course-certificate" '.$cert_bg.'>';
									 ...
									 if($certificate_title != '' || $certificate_subtitle != '') {    
									...
									if($show_certificate_issueddate == 'yes' || $authority_sign != '') {
	...
	function dt_sc_classes( $atts, $content = null ) {
		
		extract(shortcode_atts(array(
			'columns' => '',
			'post_per_page' => '-1',
			'disable_onsiteonline_filter' => '',
			'disable_popularfeatured_filter' => '',
			'disable_listgridview' => '',
		), $atts));
		...
		$out = '<div class="dt-sc-classes-list-shortcode" data-postlayout="'.$columns.'" data-postperpage="'.$post_per_page.'">';
					
					$class_filter_overall = $onsiteonline_class = $popularfeatured_class = $listgridview_class = '';
					
					if($disable_onsiteonline_filter == 'true' && $disable_popularfeatured_filter != 'true') {
						$onsiteonline_class = 'hidden';
						$class_filter_overall = 'dt-sc-classes-filter-singleitem';
					} else if($disable_onsiteonline_filter != 'true' && $disable_popularfeatured_filter == 'true') {
						$popularfeatured_class = 'hidden';
						$class_filter_overall = 'dt-sc-classes-filter-singleitem';
					} else if($disable_onsiteonline_filter == 'true' && $disable_popularfeatured_filter == 'true') {
						$class_filter_overall = 'hidden';
					}
					
					if($disable_listgridview == 'true') {
						$listgridview_class = 'hidden';
					}
					
					$out .= '<div class="dt-sc-classes-filters '.$class_filter_overall.'">';
						
						$out .= '<div class="classes-type '.$onsiteonline_class.'">
									<a class="class-type class-type-all active" data-postid="'.$post->ID.'" data-class_type="all"> <span> </span>'.__('All','dt_themes').'</a>
									<a class="class-type class-type-onsite" data-postid="'.$post->ID.'" data-class_type="onsite"> <span> </span>'.__('Onsite','dt_themes').'</a>
									<a class="class-type class-type-online" data-postid="'.$post->ID.'" data-class_type="online"> <span> </span>'.__('Online','dt_themes').'</a>
								</div>';

						$out .= '<div class="classes-items-type '.$popularfeatured_class.'">
									<a class="class-item-type class-item-type-all active" data-postid="'.$post->ID.'" data-class_item_type="all"> <span> </span>'.__('All','dt_themes').'</a>
									<a class="class-item-type class-item-type-popular" data-postid="'.$post->ID.'" data-class_item_type="popular"> <span> </span>'.__('Popular','dt_themes').'</a>
									<a class="class-item-type class-item-type-featured" data-postid="'.$post->ID.'" data-class_item_type="featured"> <span> </span>'.__('Featured','dt_themes').'</a>
								</div>';
					
					$out .= '</div>';
					
					$out .= '<div class="classes-view-type '.$listgridview_class.'">
								<a class="class-layout class-grid-type active" data-postid="'.$post->ID.'" data-view_type="grid"> <span class="fa fa-th-large"> </span>'.__('Grid View','dt_themes').'</a>
								<a class="class-layout class-list-type" data-postid="'.$post->ID.'" data-view_type="list"> <span class="fa fa-th-list"> </span>'.__('List View','dt_themes').'</a>
							</div>';
	...
	function dt_sc_quizzes( $atts, $content = null ) {
		...
		if($quiz_ids != '') {
			...
			$args = array('post_type' => 'dt_quizes', 'posts_per_page' => -1, 'post__in' => $quiz_ids);
		} else {
			$args = array('post_type' => 'dt_quizes', 'posts_per_page' => -1);
		}
		...
		if($quiz_query->have_posts()): 
			...
			$quiz_permalink = get_permalink();
			
			$temp_class = '';
			
			$firstcls = '';
			$no = $quiz_query->current_post+1;
			if(($no%$firstcnt) == 1){ 
				$firstcls = ' first';
			}
			$temp_class = 'class="'.$article_class.' '.$firstcls.'"';
			...
			$out .= '<div '.$temp_class.'>
						...
						<article id="quiz-post-'.$quiz_id.'" class="'.implode(" ", get_post_class('dt-sc-quiz-list', $quiz_id)).'">
						
							<div class="dt-sc-quiz-thumb">
								<a href="'.$quiz_permalink.'" >';
									if(has_post_thumbnail($quiz_id)):
										$attachment_id = get_post_thumbnail_id($quiz_id);
							...
							<div class="dt-sc-quiz-details">	
							
								<h5><a href="'.$quiz_permalink.'" title="'.$quiz_title.'">'.$quiz_title.'</a></h5>';
								
								$out .= do_shortcode('[ratings id="'.$quiz_id.'"]');
								
								$quiz_subtitle = get_post_meta($quiz_id, "quiz-subtitle", true);
								if($quiz_subtitle != '') {
									$out .= '<p>'.$quiz_subtitle.'</p>';	
								}
								
								$out .= '<a class="dt-sc-viewall-quizzes" href="'.$quiz_permalink.'">'.esc_html__('View Quiz', 'dt_themes').'</a>';
