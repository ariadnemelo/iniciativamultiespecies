...
class MY_Portfolio_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,IAMD_THEME_NAME.__(' Portfolio', 'lms'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo $this->get_field_id('_post_categories'); ?>">
			<?php _e('Choose the categories you want to display (multiple selection possible)', 'lms');?></label>
        ...
        <p><label for="<?php echo $this->get_field_id('_excerpt'); ?>"><?php _e('Display title only or title &amp; excerpt', 'lms');?></label>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('_enabled_image');?>" name="<?php echo $this->get_field_name('_enabled_image');?>"
	         <?php checked($_enabled_image); ?> /> <?php _e("Show Image", 'lms');?></p>  

	    <p><label for="<?php echo $this->get_field_id('_post_count'); ?>"><?php _e('No.of posts to show:', 'lms');?></label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-portfolio-widget'><ul>";		
			 ...
			 endwhile;
			 	...
			 	echo "<li>".__('No Portfolio Entries found', 'lms')."</li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";			 
		echo $after_widget;
