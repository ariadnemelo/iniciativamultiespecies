...
function dttheme_get_single_course_page_ratings($course_id) {
	...
	echo '<div class="dt-sc-average-rating">
			...
			<span class="clientRating">'.$total_ratings.' '.esc_html__('ratings', 'dt_themes').'</span>
		</div>';
...
function dttheme_get_single_course_page_widget_leftside($course_id) {
	...
	if( function_exists('bp_is_active') ) {
		...
		if($course_group != '' && $course_group > 0 && bp_is_active('groups')) {
...
function dttheme_get_single_course_page_widget_rightside($course_id) {
	...
	// Ratings
	if( ( defined('WP_POSTRATINGS_VERSION') ) && !dttheme_option('general', 'disable-ratings-courses')) { 
		//echo "hehehe";
	...
	if($tasks_completed_percentage > 0) {
		...
		$out = '<div class="dt-sc-progress '.$bar_cls.'">
					<div data-value="'.$tasks_completed_percentage.'" style="background-color: '.$bgcolor.'; width: '.$tasks_completed_percentage.'%;" class="dt-sc-bar">
						<div class="dt-sc-bar-text">'.esc_html__('Course Progress', 'dt_themes').'<span style="background-color:'.$bgcolor.'">'.$tasks_completed_percentage.'%</span></div>
					</div>
				</div>';
...
function dt_get_users_class_status($class_id, $student_id) {
	...
	$class_courses_cnt = (!empty($class_courses)) ? count($class_courses) : 0;
	
	if($class_courses_cnt > 0) {
		foreach($class_courses as $class_course) {
			$course_status = dt_get_users_course_status($class_course, $student_id);
			if($course_status) {
				$compeleted_courses++;
			}
		}		
