...
<?php get_header();
	...
	<section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
		...
		<?php $allow_space  =  array_key_exists("grid_space",$tpl_default_settings) ? " with-space " : " no-space ";
			...
			<?php if( sizeof($categories) > 1 ) :
			 		if( array_key_exists("filter",$tpl_default_settings) && (!empty($categories)) ):
			 			...
			 			<div class="dt-sc-sorting-container">
			 				<a href="#" class="active-sort" data-filter=".all-sort"><?php esc_html_e('All', 'lms');?></a>
			...
			<!-- **Portfolio Container** -->
			<div class="dt-sc-portfolio-container gallery <?php echo esc_attr( $allow_space );?>"><?php
				$args = array();
				...
				if( have_posts() ):
					...
					while( have_posts() ):
                        ...
                        <!-- Portfolio Item -->
                        <div id="<?php echo "portfolio-{$the_id}";?>" class="<?php echo esc_attr( $temp_class.$sort.$allow_space );?>">
                        	<figure>
                        		<?php $popup = 'http'.dttheme_ssl().'﻿://placehold.it/1170x878&text=Add%20Image%20/%20Video%20%20to%20Portfolio';
                        				...
                        				if( "video" === $item ) {
                        					...
                        					if( !empty($items) ) {
                        						echo "<img src='".$portfolio_item_meta['items'][key($items)]."' width='1170' height='878'/>";	
                        					} elseif($pholder != 'on') {
                        						echo '<img src="http'.dttheme_ssl().'﻿://placehold.it/1170x878&text=Add%20Image%20/%20Video%20%20to%20Portfolio" width="1170" height="878"/>';
                        					}
                        				} else {
                                ...
                                <div class="image-overlay"> 
                                    <div class="image-overlay-details"> 
                        				...
                        				<?php endif;?>
                                        <div class="links">
                                            <a href="<?php echo esc_url( $popup );?>" data-gal="prettyPhoto[gallery]" title="<?php the_title(); ?>"> <span class="fa fa-search"> </span> </a>
                                            <a href="<?php the_permalink();?>" title="<?php the_title(); ?>"> <span class="fa fa-link"> </span> </a>
                                        </div>
                                    </div>
