	...
	if(isset($dtLMSAjaxNonce) && wp_verify_nonce($dtLMSAjaxNonce, 'dtLMSAjaxNonce')) {
		...
		if($class_item_type != 'popular') {
			...
			if( $class_query->have_posts() ): 
				echo '<div class="dt-sc-results-found">'.esc_html__('Records Found', 'lms').' : <span>'.$class_query->found_posts.'</span></div>';
				$i = 1;
				while( $class_query->have_posts() ): $class_query->the_post();
					...
					<article id="post-<?php echo esc_attr( $class_loop_id ); ?>" class="<?php echo implode(" ", get_post_class('dt-sc-custom-class-type '.$class_type_label.' '.$layout_class, $class_loop_id)); ?>">
						...
						<div class="dt-sc-class-thumb">
							...
							if($dt_class_featured == 'true') {
								?>
								<div class="featured-tag"><div><i class="fa fa-thumb-tack"></i><span><?php echo esc_html__('Featured', 'lms'); ?></span></div></div>
								<?php
							}
							?>
							...
							<span class="dt-sc-class-type <?php echo esc_attr( $class_type ); ?>"><?php echo "{$class_type_icon}".$class_type; ?></span>
						</div>			
						...
						<div class="dt-sc-class-details">	
							...
							<?php echo do_shortcode('[ratings id="'.$class_loop_id.'"]'); ?>
							...
							if($class_type == 'onsite') {
								...
								if($dt_class_enable_purchases == 'true') {
									...
									if(dt_check_student_already_registered($class_loop_id)) {
										echo '<span class="dt-sc-class-price dt-sc-class-registration"><span class="amount">'.esc_html__('Registered', 'lms').'</span></span>';	
									} else {
										if($seats_available > 0 || ($seats_available <= 0 && $dt_class_disable_purchases_regsitration != 'true')) {
											echo '<span class="dt-sc-class-price dt-sc-class-registration"><span class="amount">'.esc_html('Needs Registration', 'lms').'</span></span>';
										} else {
											echo '<span class="dt-sc-class-price dt-sc-class-registration"><span class="amount">'.esc_html__('Registration Closed', 'lms').'</span></span>';	
										}
									}
								}
							...
							if($dt_class_content_options == 'course') {
								...
								if($dt_class_courses_cnt > 0) {
									echo '<p class="dt-sc-total-courses-holder">'.$dt_class_courses_cnt.'&nbsp;'.esc_html__('Courses', 'lms').'</p>';	
									...
									if($dt_class_courses_cnt > 3) {
										echo '<a class="dt-sc-class-viewall-courses" href="#">'.esc_html__('View All Courses', 'lms').'</a>';
									}
								}
							...
							<div class="dt-sc-view-class-holder">
								<a title="<?php echo get_the_title(); ?>" href="<?php echo the_permalink(); ?>" class="dt-sc-button small"> <?php echo esc_html__('View Class', 'lms'); ?> </a>
							</div>
				...
				echo '<div class="dt-sc-info-box">'.esc_html__('No Classes Found!', 'lms').'</div>';
			endif; 
				...
				foreach($wp_class_qry as $class_item) :
					...
					<article id="post-<?php echo esc_attr( $class_loop_id ); ?>" class="<?php echo implode(" ", get_post_class('dt-sc-custom-class-type '.$class_type_label.' '.$layout_class, $class_loop_id)); ?>">
						...
						<div class="dt-sc-class-details">	
							...
							<div class="dt-sc-view-class-holder">
								<a title="<?php echo get_the_title($class_loop_id); ?>" href="<?php echo get_permalink($class_loop_id); ?>" class="dt-sc-button small"> <?php echo esc_html__('View Class', 'lms'); ?> </a>
							</div>
