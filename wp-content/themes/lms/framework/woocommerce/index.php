...
# wrap products on overview pages into an extra div for improved styling options. adds "product_on_sale" class if product is on sale
	...
	if( !function_exists('dt_woocommerce_template_loop_product_thumbnail')){
		function dt_woocommerce_template_loop_product_thumbnail() {
			...
			$image = !empty( $image ) ? $image : '<img  width="100%" height="100%" src="http'.dttheme_ssl().'://placehold.it/500x500"/>';
			...
			if ($product->is_on_sale() and $product->is_in_stock() ) :
				$out .= apply_filters('woocommerce_sale_flash', '<span class="onsale"><span>'.esc_html__( 'Sale!', 'lms' ).'</span></span>', $post, $product);
			elseif(!$product->is_in_stock()):
				$out .= apply_filters( 'woocommerce_sale_flash', '<span class="out-of-stock"><span>'.esc_html__( 'Out of Stock', 'lms' ).'</span></span>', $post, $product );
			endif;

			if( $product->is_featured() )
				$out .=  apply_filters( 'woocommerce_sale_flash', '<span class="featured"><span>'.esc_html__( 'Featured', 'lms' ).'</span></span>', $post, $product );
...
#Showing Upsell Products( You may also like)
	...
	if( !function_exists('dt_woocommerce_show_product_sale_flash') ){
		function dt_woocommerce_show_product_sale_flash() {
			...
			$out = "";
			if( $product->is_on_sale() and $product->is_in_stock() )
				$out .= '<span class="onsale">'.esc_html__('Sale!', 'lms').'</span>';
			elseif(!$product->is_in_stock())
				$out .= '<span class="out-of-stock">'.esc_html__('Out of Stock', 'lms').'</span>';

			if( $product->is_featured() )
				$out .= '<span class="featured-product">'.esc_html__('Featured', 'lms').'</span>';
			echo "{$out}";
		}
	}
...
function dt_woocommerce_before_cart() {
	echo '<p class="return-to-shop"><a class="button wc-backward" href="'.esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ).'">'.esc_html__( 'Return To Shop', 'lms' ).'</a></p><div class="dt-sc-hr-invisible-small"></div>';
}
...
if(function_exists('dttheme_yith_subscription_plugin_active') && dttheme_yith_subscription_plugin_active()) {
	...
	if( !function_exists('dttheme_shop_product_tabs') ) {
		function dttheme_shop_product_tabs($tabs = array()) {
			...
			$is_subscription_product = get_post_meta( $product->get_id(), '_ywsbs_subscription', true );
			
			// Courses available in this subscription package
			if($is_subscription_product == 'yes') {
				
				$tabs['subscription_courses'] = array(
					'title'    => esc_html__( 'Subscription Products', 'lms' ),
					'priority' => 1,
	...
	function dttheme_shop_subscription_courses_tab() {
		...
		if ( count( $classes ) > 0 ) {
			...
			echo '<h4>'.esc_html__('Classes', 'lms').'</h4>';
			
			echo '<table cellspacing="10" cellpadding="0" border="0" style="width:100%;">
					<thead>
						<tr>
							<th scope="col">#</th>
							<th scope="col" colspan="2">'.esc_html__('Class', 'lms').'</th>
							<th scope="col">'.esc_html__('Price', 'lms').'</th>
					...
					foreach ($classes as $class){
						...
						if(in_array($product->get_id(), $product_ids)) {
							...
							if($dt_class_product_id != '') {
								...
								$woo_regular_price = $product_inner->get_regular_price();
								$woo_sale_price = $product_inner->get_sale_price();	
								if($woo_sale_price != '') {
									$price_html = get_woocommerce_currency_symbol().$woo_sale_price;		
								} else {
									$price_html = get_woocommerce_currency_symbol().$woo_regular_price;
								}
							}
						   ...
						   echo '<tr>
									...
									<td>'.$price_html.'</td>
									<td><a class="dt-sc-button small filled" target="_blank" href="'.get_permalink($class_id).'">'.esc_html__('View Class', 'lms').'</a></td>
								</tr>';
		...
		if ( count( $courses ) > 0 ) {
			...
			echo '<h4>'.esc_html__('Courses', 'lms').'</h4>';
			
			echo '<table cellspacing="10" cellpadding="0" border="0" style="width:100%;">
					<thead>
						<tr>
							<th scope="col">#</th>
							<th scope="col" colspan="2">'.esc_html__('Course', 'lms').'</th>
							<th scope="col">'.esc_html__('Price', 'lms').'</th>
							<th scope="col">'.esc_html__('Option', 'lms').'</th>
						</tr>
					</thead>
					...
					foreach ($courses as $course){
						...
						if(in_array($product->get_id(), $product_ids)) {
						   ...
						   echo '<tr>
									...
									<td>'.$price_html.'</td>
									<td><a class="dt-sc-button small filled" target="_blank" href="'.get_permalink($course_id).'">'.esc_html__('View Course', 'lms').'</a></td>
								</tr>';
...
add_action( 'woocommerce_before_subcategory', 'dttheme_woo_before_subcategory', 5);
function dttheme_woo_before_subcategory() {
	...
	echo '<div class="product-wrapper '.$product_container_class.'">';
...
add_action( 'woocommerce_after_subcategory', 'dttheme_woo_after_subcategory', 10);
function dttheme_woo_after_subcategory() {
	echo '</div></div>';
}
