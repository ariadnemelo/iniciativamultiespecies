...
echo '<h3 class="border-title">'.esc_html__('Available Subscriptions', 'lms').'<span></span></h3>';
...
if($products->have_posts()):
	...
	while($products->have_posts()): $products->the_post();
		...
		if( ! $product->is_in_stock() || in_array( $product->product_type, array('external','grouped','variable') ) ){
			$add_to_cart = '<a href="'. get_permalink() .'" rel="nofollow" data-product_id="'.$product_id.'" class="dt-sc-button button product_type_'.$product->product_type.'"><span class="fa fa-sliders"></span> '.esc_html__('View Details', 'lms').'</a>';
		} else {
			$add_to_cart = '<a href="'. apply_filters( 'add_to_cart_url', esc_url( $product->add_to_cart_url() ) ) .'" rel="nofollow" data-product_id="'.$product_id.'" class="dt-sc-button button add_to_cart_button ajax_add_to_cart product_type_'.$product->product_type.'"><span class="fa fa-shopping-cart"></span> '.esc_html__('Add to Cart', 'lms').'</a>';
		}
			...
			echo '<div class="product-container">
					...
					echo '<div class="product-thumb">
								...
								if($product->is_featured()) { 
									echo apply_filters( 'woocommerce_sale_flash', '<span class="featured"><span>'.esc_html__( 'Featured', 'lms' ).'</span></span>', $post, $product ); 
								}
								if($product->is_on_sale() and $product->is_in_stock()) { 
									echo apply_filters('woocommerce_sale_flash', '<span class="onsale"><span>'.esc_html__( 'Sale!', 'lms' ).'</span></span>', $post, $product); 
								} elseif(!$product->is_in_stock()) { 
									echo apply_filters( 'woocommerce_sale_flash', '<span class="out-of-stock"><span>'.esc_html__( 'Out of Stock', 'lms' ).'</span></span>', $post, $product ); 
								}
					echo '</div>';
